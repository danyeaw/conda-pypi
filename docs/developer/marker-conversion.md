...
When building `.conda` packages from wheel `METADATA` ({py:func}`conda_pypi.translate.requires_to_conda`), markers are not encoded as `[when="…"]` (conda's `MatchSpec` does not parse `when` today). However, `extra == "…"` is split into the extras map. Other marker dimensions are omitted from `depends`.
